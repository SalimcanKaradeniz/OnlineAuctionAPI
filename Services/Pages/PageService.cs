...
namespace OnlineAuction.Services.Pages
{
    public class PageService : IPageService
    {
        ...
        public List<OnlineAuction.Data.Models.PagesModel> GetPages()
        {
            ...
            if (pages.Any())
            {
                var pageIds = pages.Select(x => x.Id);

                var viewPage = (from p in pages
                                select new PagesModel()
                                {
                                    ...
                                    IsMain = p.IsMain,
                                    CreatedAt = p.CreatedAt,
                                    SubPages = (from sp in _unitOfWork.GetRepository<OnlineAuction.Data.DbEntity.Pages>().GetAll(x => x.ParentId == p.Id, include: x => x.Include(pb => pb.PageBanners).Include(psf => psf.PageSpecification)).ToList()
        ...
        public ReturnModel<object> PageIsActiveUpdate(PageRequestModel model)
        {
            ReturnModel<object> returnModel = new ReturnModel<object>();
