...
namespace MailGunIntegration.Override
{
    public class OverrideMailGunSender
    {
        private readonly string _apiKey;
        private readonly string _domainName;
        private HttpClient _httpClient;

        public OverrideMailGunSender(string domainName, string apiKey, MailGunRegion mailGunRegion = MailGunRegion.USA)
        {
        ...
        public ApiResponse<MailgunResponse> Send(IFluentEmail email, CancellationToken? token = null)
        {
            return SendAsync(email, token).GetAwaiter().GetResult();
        }

        public async Task<ApiResponse<MailgunResponse>> SendAsync(IFluentEmail email, CancellationToken? token = null)
        {
